...
def is_target_referece(mapped_ref_name):
    if re.match(r'chr\d', mapped_ref_name) or \
       re.match(r'chrX', mapped_ref_name) or \
       re.match(r'chrY', mapped_ref_name):
        return True


def evaluate_STAMPs(bam_file, minimal_mapping_quality, out_root):
    """to be filled"""
    STAMPs = dict()
    ...
    for read in f.fetch(until_eof = True):
        if read.mapping_quality >= minimal_mapping_quality:
            ...
            if cell_barcode in STAMPs:
                if is_target_referece(mapped_ref_name):
                    STAMPs[cell_barcode][0] += 1
                elif mapped_ref_name.startswith('chrM'):
                        STAMPs[cell_barcode][1] += 1
                elif mapped_ref_name.startswith('lentiGuide'):
                    STAMPs[cell_barcode][2] += 1
            else:
                STAMPs[cell_barcode] = [int(), int(), int()]
                if is_target_referece(mapped_ref_name):
                    STAMPs[cell_barcode][0] += 1
                elif mapped_ref_name.startswith('chrM'):
                        STAMPs[cell_barcode][1] += 1
    ...
    STAMPs_sorted = sorted(STAMPs_less.items(),
                           key=lambda STAMPs_less: STAMPs_less[1],
                           reverse = True)

    total_reads = sum([sum(STAMPs[i]) for i in STAMPs])

    f_output = open(out_root + '/' + 'STAMPs_info_mapQ' + str(minimal_mapping_quality) +
        '.txt','w')
    ...
    for i in STAMPs_sorted:
        ...
        STAMPs_info = '\t'.join([str(count), j.decode(), str(STAMPs[j][0]),
            str(STAMPs[j][1]), str(STAMPs[j][2]), str(sum(STAMPs[j])),
            str(total_reads)])
        f_output.write(STAMPs_info + '\n')

    f_output.close()

    saturation_plot_y_before_cumulation = [sum(STAMPs[i[0]]) / \
                                           total_reads for i in STAMPs_sorted]
    ...
    with PdfPages(bam_file + '_mapQ' + str(minimal_mapping_quality) +
                  '_STAMPs_saturation_plot.pdf') as pdf:
...
if __name__ == "__main__":
    if is_bam_sorted_by_coordinate(sys.argv[1]):
        ...
        evaluate_STAMPs(sys.argv[1], mapping_quality_threshold, sys.argv[2])
