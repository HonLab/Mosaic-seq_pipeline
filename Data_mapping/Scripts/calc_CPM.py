...
f_output = open(sys.argv[1].replace('.txt', '') + '_CPM.txt', 'w')
...
for j, i in enumerate(gene_list):
    ...
    for jj, ii in enumerate(barcode_list):
        ...
        CPM = barcodes[ii][j] / total_reads_of_each_barcode[jj] * 1000000
        expr_of_each_transcript_of_each_barcode.append(CPM)
